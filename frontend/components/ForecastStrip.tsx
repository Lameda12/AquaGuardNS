    ...
    <div className="bg-slate-900 rounded-xl p-4">
      ...
      <div className="grid grid-cols-7 gap-2">
        {forecast.map((day) => (
          <div
            key={day.date}
            className={`rounded-lg border p-2 text-center ${STATUS_STYLES[day.status] ?? STATUS_STYLES.normal}`}
          >
            ...
            <div className="text-xs mt-1 opacity-80">{STATUS_LABEL[day.status] ?? day.status}</div>
          </div>
        ))}
      </div>
    </div>
